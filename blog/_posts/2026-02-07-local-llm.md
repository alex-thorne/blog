...
sitemap: false
categories: [Code, AI]
tags: [python, rag, ollama, privacy, local-first]
---
...
So I put together this little `local-llm` project as scaffoling to create a harness around locally run models indexing my personal notes. It uses **Ollama** for the heavy lifting (embeddings and chat) and **Chroma** for the vector store, wrapped in a Python package that handles the glue logic.

It uses **Ollama** for embeddings and chat, **Chroma** for the vector store, and a small Python package to glue them together.
